...
func (mutant *mutant) IsMutant() (bool, error) {
	...
	isMutant := mutant.counter >= MAX_SUBSEQUENCES
	// rr = controller.mutantStatsService.AddStats(isMutant)
	return isMutant, nil
}
...
func (mutant *mutant) verifySequence(sequence []byte) int {
	...
	keepSequence := true
	lastIndex := 0 //Prevent ruse cases when we know that this will fail
